 ...
 * 
 * Last-Modified: Sat, 05 Nov 2011 15:41:04 GMT
 */
(function() {
  /*
  ...
  */
  DOMUtils = {
    ...
    findUpstreamNode: function(start, test, end, strict) {
      ...
      cursor = start;
      while ((cursor.parentNode != null) && cursor !== end && (!strict || DOMUtils.isIsolateNode(cursor))) {
        result = test(cursor);
        if (result != null) {
          return result;
        }
    ...
    _surroundWithInline: function(range, wrapNode) {
      ...
      if (startContainer === endContainer && DOMUtils.isDataNode(startContainer)) {
        if (startOffset === 0 && endOffset === DOMUtils.getNodeLength(startContainer)) {
          ...
          return extracted.appendChild(DOMUtils.createElementFromHTML('<sup>HelloHelloHello</sup>'));
        }
      } else {
