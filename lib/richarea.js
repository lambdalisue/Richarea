 ...
 * 
 * Last-Modified: Fri, 02 Dec 2011 10:11:49 GMT
 */
(function() {
  ...
  */
  ...
  Event = (function() {
    ...
    Event.prototype.addEventListener = function(type, listener) {
      if (!(this._eventListeners[type] != null)) this._eventListeners[type] = [];
    ...
    Event.prototype.fire = function(event) {
      ...
      if (!(event.target != null)) event.target = this.target || this;
      if (!(event.type != null)) {
    ...
    Event.addEventListener = function(target, type, listener) {
      var event;
      if (!(target.__event != null)) target.__event = new Event(target);
  ...
  */
  ...
  DOMUtils = {
    ...
    isNode: function(node) {
      if ((window.Node != null) && node instanceof Node) {
        return true;
      } else if (typeof node === 'object' && typeof node.nodeType === 'number' && typeof node.nodeName === 'string') {
        return true;
      }
      return false;
    },
    isElement: function(node) {
      if ((window.Element != null) && node instanceof Element) {
        return true;
      } else if (typeof node === 'object' && typeof node.nodeType === 1 && typeof node.nodeName === 'string') {
        return true;
      }
      return false;
    },
    isHTMLBodyElement: function(node) {
      return DOMUtils.isNode(node) && node.nodeName === 'BODY';
    },
    ...
    isVisibleNode: function(node) {
      ...
      if (DOMUtils.isDataNode(node)) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          child = _ref[_i];
          if (!DOMUtils.isVisibleNode(child)) return false;
        }
        return true;
      }
    },
    ...
    findUpstreamNode: function(start, test, endTest) {
      ...
      while (cursor != null) {
        ...
        if ((endTest != null) && endTest(cursor)) return null;
      }
      return null;
    },
    ...
    extractDataNode: function(node, start, end) {
      ...
      if (start === end || (start === 0 && end === text.length)) return node;
    ...
    _tidyInline: function(root) {
      ...
      while (cursor != null) {
        ...
        if (DOMUtils.isContainerNode(cursor)) {
          ...
          if (found != null) Surround.remove(cursor);
        }
        if (DOMUtils.isInlineNode(cursor)) HTMLTidy._tidyInline(cursor);
        _results.push(cursor = next);
      }
      return _results;
    },
    _tidyBlock: function(root) {
      var cursor, next, nextSibling, nextSiblingFragment, offended, parentNode, _ref, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _results;
  ...
  */
  ...
  Selection = (function() {
    ...
    function Selection(document) {
      ...
      if (!(this.document.createRange != null) && (window.DOMRange != null)) {
        this.document.createRange = function() {
          return new DOMRange(_this.document);
        };
        selection = new DOMSelection(this.document);
        this.window.getSelection = function() {
  ...
  */
  ...
  Surround = {
    ...
    _container: function(node, cover) {
      ...
      found = DOMUtils.findUpstreamNode(node, test);
      node = Surround.out(found || node, cover);
    ...
    _block: function(node, cover, paragraph) {
      var end, found, prerange, start, test;
      if (paragraph == null) paragraph = true;
      test = function(node) {
    ...
    _inline: function(root, start, end, cover) {
      ...
      if (found != null) {
        ...
        if (Richarea.detector.browser === 'Explorer') {
        ...
        if (removeMode) {
          for (_j = 0, _len2 = reports.length; _j < _len2; _j++) {
            report = reports[_j];
            if (report.found != null) Surround.remove(report.found);
          }
        } else {
          for (_k = 0, _len3 = reports.length; _k < _len3; _k++) {
            report = reports[_k];
            if (!(report.found != null)) Surround.out(report.node, cover);
          }
        }
        prerange = new Prerange;
        prerange.setStart(start);
  ...
  */
  ...
  ContentEditable = (function() {
    ...
    function ContentEditable(iframe) {
      ...
      this.bind('ready', function() {
        ...
        _this.document = _this.iframe.contentDocument || _this.window.document;
        if (!(_this.document.body != null)) {
          _this.document.writeln('<body></body>');
        }
        _this.body = _this.document.body;
        _this.body.style.cursor = 'text';
        _this.body.style.height = '100%';
        if (Richarea.detector.browser === 'Explorer' && Richarea.detector.version < 9) {
          updateBodyHeight = function() {
            return _this.body.style.height = "" + _this.iframe.offsetHeight + "px";
          };
          setTimeout(function() {
            var _ref, _ref2;
            if (((_ref = _this.iframe) != null ? _ref.offsetHeight : void 0) !== ((_ref2 = _this.body) != null ? _ref2.offsetHeight : void 0)) {
              updateBodyHeight();
            }
            return setTimeout(arguments.callee, 100);
          }, 100);
        }
        if (_this.body.spellcheck != null) _this.body.spellcheck = false;
        if (_this.body.contentEditable != null) {
          _this.body.contentEditable = true;
        } else if (_this.document.designMode != null) {
          _this.document.designMode = 'On';
        }
        Event.bind(_this.body, 'focus click dblclick mousedown mousemove mouseup keydown keypress keyup blur paste', function(e) {
          return _this.fire(e);
        });
        _this.bind('focus', function() {
          return _this.iframe.setAttribute('previousContent', _this.getValue());
        });
        _this.bind('focus click dblclick mousedown mousemove mouseup keydown keyup paste blur', function() {
          return _this.update();
        });
        return _this.bind('keydown', function(e) {
          var key;
          ...
          if (key === 9 && !e.ctrlKey && !e.altKey) {
            ...
            _this.execCommand(e.shiftKey ? 'outdent' : 'indent');
            _this.update();
            return false;
          }
          return true;
      ...
      if (this.iframe.getAttribute('src') != null) {
        if (!(this.iframe.addEventListener != null)) {
          this.iframe.attachEvent('onreadystatechange', function() {
            if (_this.iframe.readyState === 'complete') {
              _this.iframe.detachEvent('onreadystatechange', arguments.callee);
              return _this.fire('ready');
            }
          });
        } else {
          Event.bind(this.iframe, 'load', function() {
            return _this.fire('ready');
          });
        }
      } else {
    ...
    ContentEditable.prototype.getValue = function() {
      if (this.window != null) return this.body.innerHTML;
    };
  ...
  this.Richarea = (function() {
    ...
    function Richarea(iframe) {
      ...
      this.raw.ready(function() {
        _this.selection = new Selection(_this.raw.document);
        _this.raw.bind('focus click dblclick mousedown mousemove mouseup keydown keypress keyup paste blur change', function(e) {
          return _this.fire(e);
        });
    ...
    Richarea.prototype.getUpstreamNodeListOfSelection = function() {
      ...
      nodelist = [];
      fn = function(node) {
        if (!DOMUtils.isDataNode(node)) return nodelist.push(node);
      };
    ...
    Richarea.prototype.getUpstreamNodeTagNameListOfSelection = function() {
      ...
      namelist = [];
      fn = function(node) {
        if (!DOMUtils.isDataNode(node)) return namelist.push(node.tagName);
      };
      if (this.applyToAllUpstreamNodeOfSelection(fn)) {
    ...
    Richarea.prototype.getSelectedContent = function() {
      ...
      if (selection.rangeCount > 0 && !selection.isCollapsed) {
        range = selection.getRangeAt(0);
        if (range != null) return range.cloneContents();
      }
      return null;
    };
