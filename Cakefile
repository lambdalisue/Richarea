...
task 'pack', 'Create release pack', ->
  jspacker = spawn 'jspacker', ['-e62', '-i', 'lib/richarea.js', '-o', 'lib/richarea.min.js']
  jspacker.stdout.on 'data', (data) -> print data.toString()
  jspacker.stderr.on 'data', (data) -> print data.toString()
  jspacker.on 'exit', (status) -> callback?() if status is 0
